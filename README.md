...
目录下维护产品选型、首次使用、接口接线、差分链路、平台接入和故障排查内容。
...
│   ├── conf.py              # Sphinx 配置
│   ├── index.rst            # 文档首页
│   ├── 产品矩阵/            # 产品选型、五个 SKU 产品页和完整参数对比
│   ├── RTK系列/             # RTK 快速开始、连接、差分、平台接入和维护
│   ├── GNSS系列/            # HM-G51B 用户手册
│   └── _static/             # 站点公共样式
...
- 完整公开参数由 `source/产品矩阵/概述.md` 统一维护；SKU 页和快速开始只保留选型或操作必需值。
- 新页面加入导航前运行严格 HTML 构建和链接检查，旧网址在删除前先保留兼容入口。
