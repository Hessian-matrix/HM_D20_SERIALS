...
:::{note} 名称说明
APM 是历史上常见的硬件或项目称呼；本文统一使用当前软件项目名称 **ArduPilot**。
:::

固件选择见[固件与输出协议选择](../05-基本使用/固件与输出协议选择.md)，完整 8-pin 定义见[D20 接口与接线](../05-基本使用/D20接口与接线.md)。
...
保持设备和飞控断电，并按[完整 8-pin 定义](../05-基本使用/D20接口与接线.md)逐线核对。D20 只允许使用 5 V ±0.5 V 供电；禁止正负极反接，禁止使用错误电压，禁止把飞控电源正极接到 D20 TX、RX 或其他信号线，否则可能损坏设备。
:::
...
飞控持续识别 GPS，卫星数和定位数据正常更新；开阔环境能够进入 **RTK 固定解**，且解锁飞行前状态稳定、无异常跳变。
:::

若飞控无法识别 D20，先检查无人机版固件、115200 bps 波特率、3.3 V UART 电平兼容性和 TX/RX 接线。有定位输出但无法进入固定解时，按[RTK 状态与固定解验证](../05-基本使用/RTK状态与Fixed验证.md)检查差分配置和现场条件。
